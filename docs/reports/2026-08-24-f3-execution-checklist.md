...
Archived: 2026-08-24 after F3 closure. This is the complete historical F3
execution checklist; active work now lives in `docs/current-focus.md` under the
adopted `TODO-ALTERNATIVE.md` roadmap.

## Active Slice

F3 - External API Gateway is complete. All eight batches and the four original
F3 exit outcomes have passed their required source, contract, release,
rebuilt-image, health, live-client, scan, and documentation proofs.

The F2 exit and F3 entry gates are complete, every required gateway decision is
frozen, and Batches 1 through 8 have passed their focused and final checks. All Batch 5
Auth, User, Settings, Product, Blog, scoped Taxonomy, Order, and Media slices
now expose their selected operations with the
frozen transport/actor/allowlist rules and checked OpenAPI routes. Batch 4's
route/input/envelope/idempotency owners, Batch 3's outbound trust, verified
context, internal clients, Auth resolver, causal propagation, and readiness
foundation, and Batch 6's nine-runtime/private-release boundary remain in
force. `TODO.md` remains the milestone board. This file
is the single, complete F3 checklist and work order; it is not evidence that an
unchecked item is implemented.
...
| Runtime/tooling                | One root inventory, shared Dockerfile/Bake graph, and sequential backend runner own nine backend runtimes, eight hybrid services, and seven Prisma services.                     |

## Classified Findings From The Source Audit

| Area                                           | Classification                        | Evidence-backed consequence for F3                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway application                            | confirmed defect                      | Batches 1, 5, and 6 provide the HTTP-only application, selected route/OpenAPI surface, shared image, and local/release runtime boundary; Batch 7 provides the generated external client.                                                                                                                                                                                                                                                                                                                       |
...
| Shared Joi schema composition                  | confirmed defect                      | The first correction pinned only grpc-auth. The final Batch 6 boot then proved hybrid applications/production-injected config could still resolve Joi 18.0.2 while gateway/grpc-auth resolved 18.0.1, crashing during schema composition at `SVC_NAME`. All nine applications plus config and grpc-auth now pin the selected 18.0.1 exactly; the inventory test enforces that single exact version, the Docker runtime verifier loads every compiled env schema, and the rebuilt nine-service boot now passes. |
...
| External standards                             | confirmed defect                      | Batches 4 and 5 apply the executable policy/envelope/profile/idempotency owners to all 70 selected operations and keep a byte-checked 60-path OpenAPI artifact. Batch 7 now generates and stale-checks the separate platform-neutral external client.                                                                                                                                                                                                                                                          |
| OpenAPI reflected-parameter duplication        | confirmed defect                      | Nest reflected query DTO fields alongside explicit stricter operation parameters, leaving duplicate `(in,name)` keys and weakening generated-client determinism. The OpenAPI owner now retains the first explicit parameter, the checked artifact has no duplicate keys, and a regression preserves stricter schemas such as Media `variant=web`.                                                                                                                                                              |
| Manifest HTTP success binding                  | confirmed defect                      | The unused Batch 4 decorator documented the manifest status but did not apply it at runtime, so Nest would default all POST adapters to 201. The decorator now applies the manifest-owned status, with login/refresh/logout route tests proving 200.                                                                                                                                                                                                                                                           |
| Product list protobuf omission                 | confirmed defect                      | The gateway adapter manufactured empty optional `categoryId`/`status` values, which the product gRPC DTO correctly rejected. Batch 8 now sends sparse public/admin list requests; focused, direct-source, and complete live-profile proofs pass.                                                                                                                                                                                                                                                               |
| Idempotent browser logout replay               | confirmed defect                      | Batch 4 replay retained only Location/ETag, so a lost logout response could replay JSON without repeating cookie deletion. Replay now accepts only the exact secret-free refresh-cookie deletion; live cookie values remain invalid and never enter Redis.                                                                                                                                                                                                                                                     |
...
| Current web integration                        | confirmed defect                      | Resolved in Batch 7: the thin Next BFF uses the generated gateway client and one configured gateway base/client/origin, refresh uses POST and relays rotation/expiry, logout clears matching cookie/local state, failed concurrent refresh rejects all waiters, product/taxonomy adapters preserve current UI contracts, and the focused harness covers those behaviors.                                                                                                                                       |
...
| Personal Order/Media actor policy              | confirmed defect                      | The frozen route table says user-only, but the executable records used generic authenticated roles and relied on domain services to reject or constrain admins. Batch 5 now applies a user-only gateway policy while preserving each downstream check.                                                                                                                                                                                                                                                         |
| Release health                                 | confirmed defect                      | Resolved in Batch 6 with explicit HTTP-only operational-health metadata, sanitized shared responses, production `GATEWAY_ONLY` tests, and unchanged domain-route denial.                                                                                                                                                                                                                                                                                                                                       |
| Media transport                                | confirmed defect                      | Resolved across Batches 5-6: the fixed internal HTTP origin and sole stream proxy use private `media-service:3007`; release removes media host ports and opens only media's already-public HTTP render/health behavior while private routes remain protected.                                                                                                                                                                                                                                                  |
| Media fixed/dynamic route collision            | confirmed defect                      | Registering generic `GET /admin/media/:id` before the fixed library GET routes caused lane names to be parsed as UUID IDs. The generic controller now registers after all fixed lane prefixes, with an HTTP regression test.                                                                                                                                                                                                                                                                                   |
| Release exposure                               | confirmed defect                      | Resolved in Batch 6: rendered release Compose publishes gateway port 3002 and bundled MinIO data port 9000 only; backend HTTP/gRPC, PostgreSQL, Redis, and MinIO console ports remain private.                                                                                                                                                                                                                                                                                                                 |
| Release image archive                          | confirmed defect                      | Resolved in Batch 6: the script derives `$repoRoot` from `$PSScriptRoot`, enumerates all nine runtime images plus infrastructure, checks `docker save`, and supports non-destructive `-WhatIf` verification.                                                                                                                                                                                                                                                                                                   |
| Refresh-rotation task                          | stale implementation or documentation | Resolved in Batch 7 by preserving gateway/auth-owned cookie rotation and authoritative expiry while correcting the caller to POST; no second token rotation or revocation owner was added.                                                                                                                                                                                                                                                                                                                     |
...
- [x] Add `/api/v1/auth` routes for register, login, refresh, logout, and
      ...
      token logic.
      Evidence: five checked OpenAPI paths, 11 signed-client/HTTP Auth tests,
      two cookie-attribute tests, and the final 171-test gateway suite pass.
- [x] Add only the user self/admin routes selected by the manifest, retaining
      user-service enforcement. Do not promise admin listing unless the
      additive ListUsers path was completed.
      Evidence: four User operations across three checked paths and six
      focused signed-client/HTTP tests pass; list metadata is unpaginated.
- [x] Add public/admin settings routes through the existing typed settings
      client and an explicit external key allowlist; never expose secret/trust
      configuration or the internal bootstrap RPC.
      Evidence: three Settings operations across two checked paths and six
      focused signed-client/HTTP tests pass with fixed `default` environment.
- [x] Add public and explicitly selected admin product routes. Force ACTIVE,
      ...
      Evidence: all 15 selected Product operations are checked OpenAPI/runtime
      routes; 7 focused signed-client/HTTP tests plus module/OpenAPI checks pass.
      Public lifecycle controls fail closed, widened public replies are rejected,
      and sparse PATCH forwarding preserves `false` while mapping external
      `content` to internal `description`.
- [x] Add supported public/admin blog routes without inventing an admin draft
      list or get-by-ID contract that does not exist.
      Evidence: all five selected Blog operations are runtime/OpenAPI routes;
      focused tests reject non-PUBLISHED public replies, preserve sparse
      updates, and prove no admin read methods or paths were invented.
- [x] Add product/blog taxonomy routes through `ProductTaxonomyService` and
      `BlogTaxonomyService`. Do not expose raw global taxonomy writes where the
      domain facade owns scope validation.
      Evidence: ten scoped Taxonomy operations are runtime/OpenAPI routes;
      focused service/HTTP tests prove facade selection, required kind, absent
      external scope/system controls, and wrong-scope response rejection.
- [x] Add cart, checkout, order read, and admin status routes through the
      ...
      Evidence: eight selected Order operations are runtime/OpenAPI routes;
      6 focused adapter/HTTP tests plus 12 manifest/module checks pass. Public
      input cannot select a user, legacy request IDs are blank and ignored,
      response ownership is checked, personal routes use a user-only policy,
      and no admin list/read route exists.
- [x] Add only the media lanes backed by the completed internal contracts.
      ...
      Evidence: all 19 selected Media JSON operations are runtime/OpenAPI
      routes; focused adapter/HTTP tests prove fixed lane methods, server-owned
      access/visibility, owned-row identity, user-only owned access, public
      two-step deletion, and the absence of one-step public delete.
- [x] Proxy `/api/v1/media/render/:id` as the sole byte-stream transport
      ...
      Evidence: the startup schema/env examples require one origin-only
      `MEDIA_RENDER_HTTP_URL`; three live ephemeral-upstream tests prove exact
      path/variant construction, request-ID/header/byte/status forwarding, no
      redirect following or internal Location leak, and input rejection.
- [x] Apply JSON envelopes, query profile, idempotency/retry declaration,
      ...
      Evidence: the 70-operation manifest exactly equals the landed OpenAPI
      operation-ID set; route decorators own status/auth/retry documentation,
      JSON controllers use the shared envelopes, and render remains the sole
      explicitly binary response.
- [x] Generate and snapshot the checked OpenAPI document as each domain lands.
      ...
      Evidence: the deterministic artifact contains all 70 operations across
      60 paths. `openapi:check`, lint, type-check, gateway build, 41 suites, and
      all 171 tests pass; `git diff --check` is clean.

### Batch 6 - Runtime And Release Boundary

- [x] Migrate the root backend inventory before adding gateway metadata. Keep
      ...
      Evidence: the canonical inventory now records `transport` and staged
      Compose admission, contains the HTTP-only gateway without a fake gRPC
      port, and derives nine runtime, eight hybrid, seven Prisma, and current
      Compose-admitted views in `scripts/backend.mjs`.
- [x] Update inventory/tooling/config/grpc-auth tests that currently assume
      ...
      Evidence: tooling validates 17 unique runtime ports and the HTTP-only
      gateway shape; config health/logging checks cover all runtimes while
      direct-service HTTP and gRPC-auth wiring checks select the eight hybrid
      services. All eleven runtime Joi schema owners also pin one exact version,
      and the compiled-runtime verifier loads every service env schema so mixed
      schema versions fail during image construction rather than container
      startup. The 37 tooling, 76 config, and 119 grpc-auth tests pass. The
      repository-wide backend lint gate also passes after aligning the stale
      media-service default-project file cap with the existing peer-service
      value; no lint rule was weakened.
- [x] Add the gateway to the shared Dockerfile/Bake graph and sequential runner.
      Do not create a separate Dockerfile, a second service list, or parallel
      builds.
      Evidence: the shared Dockerfile builds and packages `@nebula/gateway` as
      `gateway-runtime`, the Bake group renders all nine targets, and the
      existing inventory-backed runner includes gateway sequentially. Gateway
      type-check and source build pass. The initial user-run sequential image
      graph proof completed all nine targets and confirmed all nine
      inventory-derived tags. Because later Batch 6 source changes supersede
      those image bytes, the final cached image/live proof remains the explicit
      user-run checkpoint below rather than treating the earlier manifest as
      current-source evidence. The first final live attempt exposed the
      cross-workspace Joi resolution defect described above. After aligning all
      schema owners and extending build-time verification, the user-run rebuild
      brought all nine application runtimes to readiness, both demo seeds were
      idempotently reported as existing, and `backend:boot` completed. The
      independent read-only `pnpm backend:health` follow-up reports all nine
      runtimes `ok`.
- [x] Add local and release Compose gateway services with port 3002, validated
      ...
      Evidence: both Compose files now own the gateway runtime. It uses all
      eight fixed Docker-DNS gRPC targets, the outbound-only key map, validated
      registry/config values, gateway Redis, private media render, and only
      Auth/Redis health dependencies; both rendered configurations validate.
- [x] Keep all backend HTTP/gRPC ports published in local Compose for diagnosis.
      ...
      Evidence: rendered local Compose publishes all eight hybrid HTTP/gRPC
      pairs plus gateway 3002. Rendered release Compose publishes only gateway
      3002 and bundled MinIO data 9000; backend application, PostgreSQL, Redis,
      and MinIO console ports have no host bindings.
- [x] Preserve the existing release `PUBLIC_MODE=GATEWAY_ONLY` default. Add a
      ...
      Evidence: every health controller now uses `@OperationalHealth()`. The
      shared guard permits that metadata for HTTP only, rejects its use as an
      unsigned RPC bypass, rejects ordinary public/private HTTP in production
      `GATEWAY_ONLY`, and retains the sanitized shared health shape.
- [x] Separately prove unsigned gRPC is rejected, gateway-only RPCs reject an
      ...
      Evidence: the 119-test grpc-auth suite proves public RPC still requires
      S2S, service identity is denied on gateway-only policy, exact mixed
      gateway/approved-service identity works, and ordinary service v2 remains
      valid alongside required gateway v3 context.
- [x] Apply the selected private-network media render exception narrowly. If
      ...
      Evidence: release media alone overrides `PUBLIC_MODE=OPEN`, has no host
      application ports, and is reached by gateway only at the validated
      `http://media-service:3007` origin. The production guard test proves OPEN
      still cannot expose a private route.
- [x] Fix and test the undefined `$repoRoot` in
      `scripts/docker/save-release-images.ps1` before relying on the release
      archive to contain the ninth image.
      Evidence: `$repoRoot` is resolved from `$PSScriptRoot`; the script checks
      archive failure and supports `-WhatIf`. The non-destructive run resolved
      all nine inventory images plus four infrastructure images without writing
      the archive.
- [x] Add gateway/shared unit and contract commands to the root quality/CI
      ...
      Evidence: root `test:shared:backend` covers config, grpc-auth, and internal
      clients; `test:gateway:backend` covers all gateway tests plus the OpenAPI
      stale check. CI runs both explicitly. Current results are 76 config, 119
      grpc-auth, 12 client, and 171 gateway tests.
- [x] Assert rendered Compose reachability and health behavior, not only YAML
      parsing/regex. Extend the existing Docker/tooling owner and sequential
      boot flow rather than creating another workflow.
      Evidence: `captureComposeConfigurations` now renders JSON and enforces the
      local/release port, gateway target/environment/dependency/healthcheck,
      media policy, and no-`service_started` contracts. Its mutation tests and
      the real `pnpm evidence:compose:backend` run pass.

### Batch 7 - External Typed Client And Current Web Corrections

- [x] Finish and stale-check the route-derived OpenAPI JSON, then generate a
...
- [x] Add the smallest focused `apps/web` test/contract harness needed for these
      corrections; the current package has no test script or route/helper tests.
- [x] Make the current Next BFF send its configured registered public-client
      ...
      `fetch`.
- [x] Fix `apps/web/src/lib/auth/refresh.ts` to call its POST-only refresh route
      with `method: "POST"` and cover it with a regression test.
- [x] Characterize and preserve the existing rotated refresh-cookie write, then
...
- [x] Add or repoint the smallest browser logout handler/caller needed for the
...
- [x] Fix `apiFetch` so queued concurrent requests are rejected and cleared
      when refresh fails rather than remaining unresolved forever.
- [x] Add the missing product collection POST forwarding path. Preserve the
      UI's existing `{ data: ... }` create and `{ patch: ... }` update nesting,
      but map UI `content` to backend `description` for both POST and PATCH.
- [x] Replace direct auth/product/taxonomy service URL assumptions with one
      ...
      destinations.
- [x] Migrate callers to the generated client, then remove or reduce old Next
...
- [x] Cover refresh success/failure/concurrency/cookie expiry, login/logout,
      product create/update mapping, application identity, gateway error shapes,
      and removal of individual service URL assumptions.
- [x] Keep this compatibility patch surgical. F7 still owns the Vite admin,
      storefront split, and broader reuse or replacement of `apps/web`.

Batch 7 verification on 2026-08-22 passed the gateway OpenAPI stale check, the
external client's non-mutating generation check, lint, type-check, build, five
runtime contract tests, and browser/no-DOM React Native consumer type targets.
The current-web harness passed 15 focused tests across refresh, concurrency,
cookie rotation/expiry, login/logout, application/host-origin binding, gateway
error relay, product mapping, and taxonomy routing. Full web type-check passed;
full web lint completed with zero errors and 26 existing non-blocking frontend
warnings outside this compatibility slice. The 37 backend-tooling contract
tests also passed with the new CI/root commands. Static scans find no remaining
auth/product/taxonomy service URL key or internal gRPC URL assumption in
`apps/web`; its tracked example and ignored development env now contain only
the gateway base URL, registered client ID, and configured application origin.

### Batch 8 - Documentation, Proofs, And F3 Exit Gate

- [x] Document the route manifest, envelopes/errors, resource query profiles,
...
- [x] Update the exact runtime owners: the new gateway document, docs index,
...
- [x] After the behavior lands, update the actor/S2S contracts so a new ingress
...
- [x] Prove anonymous storefront, authenticated storefront user, authorized
...
- [x] Prove each external client configures only the gateway API base URL and no
      internal service URL. It may follow an issued presigned storage/CDN URL.
- [x] Prove release clients cannot reach backend HTTP/gRPC, Postgres, or the
      MinIO console, while the approved storage data plane and local diagnostic
      ports behave as documented.
- [x] Prove ordinary clients cannot directly set or override actor,
...
- [x] Prove release healthchecks work under the preserved gateway-only policy,
...
- [x] Run focused gateway/config/grpc-auth/clients/auth/web/tooling tests first;
      ...
      Final evidence: the refreshed gateway lane passes 41 suites/172 tests;
      shared config/grpc-auth/clients pass 76/119/12 tests; backend tooling
      passes 37 tests; all 17 backend type/build dependency tasks pass; API and
      proto stale checks pass; the dependency gate reports zero backend runtime
      and tooling findings; and the source gate reports zero secrets and zero
      misconfigurations. The full sequential service e2e lane passes 284 tests.
- [x] Review the complete diff and tracked-file cleanliness. Update `TODO.md`
      ...
      Final evidence: `git diff --check` reports no whitespace error (only the
      pre-existing media ESLint CRLF normalization warning). All nine rebuilt
      image tags pass the image gate with zero blocking findings; 288 Debian
      findings without available fixes remain visible and assigned to F9. The
      recreated containers match those scanned image IDs, all nine readiness
      checks pass, and the rebuilt gateway passes all six bounded F3 live
      checkpoints. The four original `TODO.md` F3 exit outcomes now agree and
      are checked.

Batch 8 source verification on 2026-08-22 passed the 41-suite/171-test gateway
lane; config, grpc-auth, and internal-client suites; the 15-test current-web
lane; 37 backend-tooling tests; the external-client lint/type/runtime/browser/
native lane; proto/OpenAPI/client stale checks; the security lane; backend lint,
types, and all nine source builds; and rendered local/release Compose evidence.
A temporary current-source gateway on port 3102 passed the bounded live proof
for readiness, anonymous/authenticated storefront, authorized admin,
registered mobile, both session transports, and disabled partner execution.

That live proof exposed and closed one confirmed defect: Product public/admin
list adapters sent empty optional protobuf UUID/enum values and received the
domain service's correct validation rejection. They now preserve omission, and
the focused regression plus direct source-to-live-service call pass. It also
closed one stale command-owner claim by adding the already-frozen root
`api:gen` and `api:check` aliases. Full evidence is in
[`docs/reports/2026-08-22-f3-exit-proof.md`](2026-08-22-f3-exit-proof.md).

Final closure on 2026-08-24 installed Trivy 0.74.0, selected the reachable
official Docker Hub/GHCR database order, and completed the source and nine-image
gates. The first image scan correctly blocked two fixable Node findings present
in the shared image dependency layer (`deepmerge-ts` and `js-yaml`). Narrow
owner-scoped patched overrides, successful Prisma regeneration, gateway tests,
and a second rebuild removed both findings without weakening scan policy. The
current containers were recreated from the scanned tags before the final
health and live-client proof. Complete evidence is recorded in the F3 exit
report linked above.
...
F3 is closed. Preserve `TODO.md` as the original-roadmap comparison, use the
adopted ordering in `TODO-ALTERNATIVE.md`, and replace this F3 execution
checklist with a new F4 current-focus plan before beginning implementation.
