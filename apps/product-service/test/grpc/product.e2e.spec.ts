...
describe("ProductService gRPC (admin required on writes)", () => {
  ...
  it("'CreateProduct works without metadata (internal call)", async () => {
    const res = await call<any>(client, "CreateProduct", {data: input})
  ...
  it("UpdateProduct (admin) changes title", async () => {
    const res = await call<any>(client, "UpdateProduct", {id, data: {title: "E2E Widget gRPC Pro"}}, mdS2S({role: "admin"}))
