...
export class DefaultProductTaxonomyInitializer implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.logger.log(`Default product category initialized: id=${defaultCatId} (env=${env})`)
    } catch (e) {
      this.logger.error("Failed to initialize default product taxonomy", e as any)
  ...
  private async ensureDefaultCategory(): Promise<string> {
    ...
    if (!id) {
      throw new BadRequestException("Failed to resolve default product category id")
    }
