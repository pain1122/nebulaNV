...
import {normalizeProductInput} from "./normalize-input"
...
export class ProductServiceImpl {
  ...
  private assertDiscountWindow(start?: Date | null, end?: Date | null) {
    if (start && end && end < start) {
      throw new BadRequestException("discount_end must be >= discount_start")
    }
  }
  ...
  // ---- Create ----
  async create(n: any) {
    const input = normalizeProductInput(n)
    ...
    this.assertDiscountWindow(input.discountStart, input.discountEnd)
  ...
  async update(id: string, n: any) {
    const patch = normalizeProductInput(n)
    ...
    this.assertDiscountWindow(patch.discountStart ?? undefined, patch.discountEnd ?? undefined)
    ...
    try {
      const data = await this.prisma.product.update({
        ...
        data: {
          ...
          discountStart: patch.discountStart ?? undefined,
          discountEnd: patch.discountEnd ?? undefined,
  ...
  async list(req: any) {
    const includeDeleted = !!req.include_deleted
  ...
  async applyDiscountBulk(req: any) {
    ...
    if (clear) {
      ...
      if (req.discount_start && req.discount_end) {
        this.assertDiscountWindow(new Date(req.discount_start), new Date(req.discount_end))
      }
    }
