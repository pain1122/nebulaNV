...
export class MediaService implements OnModuleDestroy {
  ...
  private resolveMediaContext(
    ...
    >,
    accessClass: AccessClass,
    requireSensitiveContext = true,
  ) {
    const scope = this.normalizeScope(input.scope);
    const entityType = this.normalizeContextValue(
      input.entityType,
      "entityType",
    );
    ...
    if (
      requireSensitiveContext &&
      accessClass !== "PUBLIC" &&
      (!safeTrim(input.scope ?? "") || !entityType || !entityId)
    ) {
  ...
  private createStrictSafeFilename(filename: string): string {
    ...
    return this.resolveDisplayName(
      filename,
      `strict-${randomUUID()}${extension || ".bin"}`,
    );
  }

  private createSensitiveLibraryObject(
    filename: string,
    accessClass: SensitiveAccessClass,
    displayName?: string | null,
  ) {
  ...
  private deriveSensitiveLibraryMetadata(
    accessClass: SensitiveAccessClass,
    filename: string,
    displayName?: string | null,
    originalFilename?: string | null,
  ...
  private createNumberedDisplayName(
    displayName: string,
    copyNumber: number,
  ): string {
    ...
    return this.resolveDisplayName(
      displayName,
      `${base}-(${copyNumber})${extension}`,
    );
  }

  private parseNumberedDisplayName(
    displayName: string,
    ...
    const rawNumber = displayName.slice(
      prefix.length,
  ...
  private async publicLibraryNameExists(
    scope: string,
    folderPath: string,
    displayName: string,
  ): Promise<boolean> {
  ...
  private async createNextPublicLibraryKey(
    filename: string,
    folderPath: string | null | undefined,
    displayName: string | null | undefined,
    scope: string,
  ) {
    const requested = this.createPublicLibraryKey(
      filename,
      folderPath,
      displayName,
    );
    ...
    const latestCopyNumber = latestVariant
      ? this.parseNumberedDisplayName(
          latestVariant.displayName,
          requested.displayName,
        )
      : null;
    ...
    const nextDisplayName = this.createNumberedDisplayName(
      requested.displayName,
      nextCopyNumber,
    );
    return this.createPublicLibraryKey(
      filename,
      requested.folderPath,
      nextDisplayName,
    );
  ...
  private async enforcePublicLibraryNameAvailable(
    scope: string,
    folderPath: string,
    displayName: string,
  ): Promise<void> {
    const exists = await this.publicLibraryNameExists(
      scope,
      folderPath,
      displayName,
    );
    if (exists)
      throw new BadRequestException("public_library_file_already_exists");
  }

  private assertPublicLibraryMedia(row: Media): void {
    if (
      row.accessClass !== "PUBLIC" ||
      row.visibility !== "public" ||
      !this.isUnderFolderRoot(row.path, "MEDIA_PUBLIC_FOLDER", "uploads")
    ) {
      throw new BadRequestException("not_public_library_media");
    }
  }

  private assertSensitiveLibraryMedia(
    row: Media,
    accessClass: SensitiveAccessClass,
  ): void {
    const errorCode =
      accessClass === "STRICT"
        ? "not_strict_library_media"
        : "not_protected_library_media";
  ...
  private assertSensitiveFeatureContext(
    row: Media,
    input: FeatureReadUrlInput,
    accessClass: SensitiveAccessClass,
  ): void {
    ...
    if (
      ...
      row.entityType !== context.entityType ||
      row.entityId !== context.entityId
    ) {
  ...
  private publicLibraryBaseWhere(scope: string): Prisma.MediaWhereInput {
    return {
      ...
      scope,
      path: {
        startsWith: `${this.getFolderRoot("MEDIA_PUBLIC_FOLDER", "uploads")}/`,
      },
      NOT: { status: MediaStatus.DELETED },
    };
  }

  private isInsideFolder(
    folderPath: string,
    candidateFolderPath: string,
  ): boolean {
    return (
      candidateFolderPath === folderPath ||
      candidateFolderPath.startsWith(`${folderPath}/`)
    );
  }
  ...
  private normalizePublicDeleteSelection(input: PublicLibraryDeleteInput) {
    ...
    for (const item of items) {
      ...
      if (item.type === "folder") {
        ...
        if (folderPath === "/") {
          throw new BadRequestException(
            "public_library_root_delete_not_allowed",
          );
        }
  ...
  private deleteConfirmSecret(): string {
    const secret = process.env.MEDIA_DELETE_CONFIRM_SECRET;
    if (!secret) throw new Error("MEDIA_DELETE_CONFIRM_SECRET_missing");
    return secret;
  }

  private signDeletePayload(encodedPayload: string): string {
    return createHmac("sha256", this.deleteConfirmSecret())
      .update(encodedPayload)
      .digest("base64url");
  }

  private createPublicLibraryDeleteToken(
    shape: PublicLibraryDeleteTokenShape,
  ): { confirmToken: string; expiresIn: number } {
  ...
  private verifyPublicLibraryDeleteToken(
    confirmToken: string,
  ): PublicLibraryDeleteTokenPayload {
    ...
    try {
      ...
      ) as PublicLibraryDeleteTokenPayload;
    } catch {
      throw new BadRequestException("delete_confirm_token_invalid");
    }
  ...
  private deleteTokenShapeFromPayload(
    payload: PublicLibraryDeleteTokenPayload,
  ): PublicLibraryDeleteTokenShape {
    return {
  ...
  private sameDeleteTokenShape(
    a: PublicLibraryDeleteTokenShape,
    b: PublicLibraryDeleteTokenShape,
  ): boolean {
  ...
  private async buildPublicLibraryDeletePlan(input: PublicLibraryDeleteInput) {
    ...
    const folders = selection.requestedFolderPaths.map((folderPath) => {
      const files = rows.filter((row) =>
        this.isInsideFolder(folderPath, row.folderPath),
      );
    ...
    const maxSyncDeleteFiles = this.readNumberEnv(
    ...
    const canDelete = warnings.every(
      (warning) =>
        warning.code !== "folder_not_empty_requires_recursive" &&
        warning.code !== "delete_requires_worker_queue",
    );
  ...
  private derivePublicLibraryMetadata(
    ...
    const derivedFolderPath =
      this.normalizeFilemanagerFolderPath(pathFolderPath);
    const derivedDisplayName = this.resolveDisplayName(
      filename,
      pathDisplayName,
    );
    ...
    if (displayName !== undefined) {
      const requestedDisplayName = this.resolveDisplayName(
        filename,
        displayName,
      );
  ...
  async create(dto: CreateMediaInput) {
    ...
    const libraryMetadata =
      accessClass === "PUBLIC"
        ...
        : this.deriveSensitiveLibraryMetadata(
            accessClass as SensitiveAccessClass,
            filename,
            dto.displayName,
  ...
  async browseSensitiveFilemanager(
    dto: ListMediaDto,
    accessClass: SensitiveAccessClass,
  ) {
    const take = Math.min(Math.max(dto.take ?? 50, 1), 200);
  ...
  async presignUpload(b: PresignUploadInput) {
    ...
    const libraryObject = await this.createNextPublicLibraryKey(
      filename,
      b.folderPath,
      b.displayName,
      scope,
  ...
  async presignSensitiveLibraryUpload(
    b: PresignUploadInput,
    accessClass: SensitiveAccessClass,
  ) {
    const bucket = process.env.MEDIA_S3_BUCKET || "media";
    const visibility = this.visibilityFromAccessClass(accessClass);
    ...
    const libraryObject = this.createSensitiveLibraryObject(
      filename,
      accessClass,
      b.displayName,
    );
  ...
  async finalizeUpload(input: FinalizeUploadInput) {
    ...
    if (accessClass === "PUBLIC") {
      await this.enforcePublicLibraryNameAvailable(
        context.scope,
        libraryMetadata.folderPath,
        libraryMetadata.displayName,
      );
    }
  ...
  async createReadUrl(id: string, input: CreateReadUrlInput) {
    ...
    if (
      ...
      row.scanStatus !== ScanStatus.CLEAN
    ) {
  ...
  async createStrictLibraryReadUrl(id: string, input: CreateReadUrlInput) {
    ...
    this.log.log(
      `strict_media_read_url_requested id=${row.id} ownerId=${row.ownerId} actorUserId=${input.actorUserId ?? ""}`,
    );
    return this.createReadUrl(id, input);
  }

  async openPublicRenderStream(
    id: string,
    input: CreateRenderInput = {},
  ): Promise<PublicRenderObject> {
  ...
  async deleteById(id: string) {
    ...
    if (row.storage === "s3" && row.path) {
      try {
        ...
        this.log.error(
          `media_storage_delete_failed id=${id} cause=${safeErrorName(error)}`,
        );
        throw error;
      }
    }
  ...
  async deleteStrictLibraryById(id: string) {
    ...
    this.assertSensitiveLibraryMedia(row, "STRICT");
    this.log.log(
      `strict_media_delete_requested id=${row.id} ownerId=${row.ownerId}`,
    );
    return this.deleteById(id);
  }
}
