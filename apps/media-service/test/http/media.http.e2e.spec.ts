...
const mediaDatabaseUrl =
  process.env.E2E_MEDIA_DATABASE_URL ??
  "postgresql://postgres:postgres@127.0.0.1:15432/nebula_media?schema=public";
const prisma = new PrismaClient({ datasourceUrl: mediaDatabaseUrl });
...
describe("media-service HTTP (admin-only)", () => {
  ...
  it("GET /health reports DB and storage checks", async () => {
    ...
    expect(health.checks.s2sReplay.status).toBe("ok");
  });
  ...
  it("GET /media/render/:id rejects original/raw public variants", async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${MEDIA_HTTP}/media/render/${id}?variant=original`,
      );
  ...
  it("GET /media/render/:id blocks pending, protected, and strict media", async () => {
    const suffix = Math.random().toString(36).slice(2, 8);
    const folderPath = `/test/render_block_${suffix}`;
    let pendingId: string | undefined;
    let protectedId: string | undefined;
    let strictId: string | undefined;
    ...
    try {
      ...
      const pendingRes = await fetch(
        `${MEDIA_HTTP}/media/render/${pendingId}?variant=web`,
      );
      ...
      const protectedRow = await httpJson<any>(
        ...
        {
          storage: "local",
          path: `private/objects/local-render-protected-${suffix}.webp`,
          folderPath: "/",
      ...
      const protectedRes = await fetch(
        `${MEDIA_HTTP}/media/render/${protectedId}?variant=web`,
      );
      expect(protectedRes.status).toBe(403);

      const strictRow = await httpJson<any>(
        "POST",
        `${MEDIA_HTTP}/media`,
        {
          storage: "local",
          path: `private/objects/local-render-strict-${suffix}.webp`,
          folderPath: "/",
          displayName: "local-render-strict.webp",
          filename: "local-render-strict.webp",
          mimeType: "image/webp",
          sizeBytes: 5,
          accessClass: "STRICT",
          visibility: "private",
      ...
      strictId = strictRow.data.id;

      const strictRes = await fetch(
        `${MEDIA_HTTP}/media/render/${strictId}?variant=web`,
      );
      expect(strictRes.status).toBe(403);
    } finally {
      for (const id of [pendingId, protectedId, strictId]) {
        if (!id) continue;
        await httpJson<any>("DELETE", `${MEDIA_HTTP}/media/${id}`, undefined, {
          authorization: `Bearer ${adminAccess}`,
        }).catch(() => undefined);
  ...
  it("presign -> PUT -> finalize -> GET -> LIST -> pending read-url blocked -> DELETE (admin)", async () => {
    ...
    await expect(
      httpJson<any>(
        "POST",
        `${MEDIA_HTTP}/media/public-library/${id}/read-url`,
        undefined,
        {
          authorization: `Bearer ${adminAccess}`,
        },
    ...
    const del = await httpJson<any>(
      "DELETE",
      `${MEDIA_HTTP}/media/public-library/${id}`,
      undefined,
      {
        authorization: `Bearer ${adminAccess}`,
      },
  ...
  it("requires admin for browse, read-url, and delete", async () => {
    ...
    try {
      ...
      await expect(
        httpJson<any>(
          "GET",
          `${MEDIA_HTTP}/media/browse?folderPath=${encodeURIComponent(folderPath)}`,
          undefined,
          {
            authorization: `Bearer ${userAccess}`,
          },
      ...
      await expect(
        httpJson<any>(
          "GET",
          `${MEDIA_HTTP}/media/public-library/browse?folderPath=${encodeURIComponent(folderPath)}`,
          undefined,
          {
            authorization: `Bearer ${userAccess}`,
          },
  ...
  it("public-library delete preview/confirm handles bulk and recursive folder deletes", async () => {
    ...
    try {
      ...
      const nested = await createLocalPublic(
        `${baseFolder}/nested`,
        "nested.webp",
      );
      ...
      expect(
        blockedPreview.data.warnings.some(
          (warning: any) =>
      ...
      expect(preview.data.files.map((file: any) => file.id).sort()).toEqual(
        [direct.id, nested.id].sort(),
      );
  ...
  it("my protected-library routes only expose the current user's feature-owned files", async () => {
    ...
    try {
      ...
      expect(browse.data.context.accessClass).toBe("PROTECTED");
      expect(browse.data.files.some((item: any) => item.id === own.id)).toBe(
        true,
      );
      expect(
        browse.data.files.some((item: any) => item.id === otherOwner.id),
      ).toBe(false);
      ...
      expect(
        wrongEntityBrowse.data.files.some((item: any) => item.id === own.id),
      ).toBe(false);
