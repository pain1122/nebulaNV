...
export class OrderService implements OnModuleInit {
  ...
  private async fetchProductOrThrow(productId: string): Promise<ProductRecord> {
    const res: any = await firstValueFrom(
      this.productSvc.GetProduct({ id: productId }) as any,
    );

    // try common shapes: { data: {...} } | { product: {...} } | raw
    const prod: ProductRecord | undefined =
      res?.data ?? res?.product ?? res;
  ...
  async checkout(userId: string, note?: string) {
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of cart.items) {
        const lineTotal =
          this.normalizePrice(item.unitPrice) * item.quantity;
