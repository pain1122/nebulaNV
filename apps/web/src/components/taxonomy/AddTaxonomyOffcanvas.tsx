...
export default function AddTaxonomyOffcanvas({ open, ...props }: Props) {
  if (!open) return null;
  return <OpenTaxonomyOffcanvas key={props.kind} {...props} />;
}

function OpenTaxonomyOffcanvas({
  onClose,
  kind,
  titleLabel,
  parentOptions = [],
  ...
  async function submit() {
    ...
    try {
      ...
      const createdName =
        stringField(data, "name") ?? stringField(data, "title") ?? name.trim();
      const createdSlug = stringField(data, "slug") ?? slugifyFa(slug);
      ...
      setError(errorMessage(e, "خطا در ثبت"));
    } finally {
      setSubmitting(false);
    }
  }
            ...
            {showParent ? (
              <div className="mb-3">
                ...
                <TagifyInput
                  ...
                  value={parent}
                  onChange={setParent}
                  whitelist={parentOptions}
                  maxTags={1}
                  dropdownEnabled={1}
                  enforceWhitelist={true}
                />
                <div className="form-text">
                  اگر دسته مادر ندارید، خالی بگذارید.
                </div>
              </div>
            ) : null}
