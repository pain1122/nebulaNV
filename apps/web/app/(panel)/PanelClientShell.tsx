...
type SimpleBarConstructor = new (element: HTMLElement) => unknown;
...
declare global {
  interface Window {
    ...
    Waves?: { init: () => void };
    SimpleBar?: SimpleBarConstructor;
  }
}
...
export default function PanelClientShell({ children }: Props) {
  ...
  useEffect(() => {
    ...
    const defaultAttribute = storedDefault
      ? (JSON.parse(storedDefault) as Record<string, string>)
      : {};
    ...
    const searchInputResp = document.getElementById(
      "search-options-reponsive",
    ) as HTMLInputElement | null;
    const searchDropdownResp = document.getElementById(
    ...
    // ---------- 10) Preloader ----------
    const preloaderMode =
      sessionStorage.getItem("data-preloader") ?? initial["data-preloader"];
    ...
    return () => {
      ...
      if (topnavHamburger)
        topnavHamburger.removeEventListener("click", handleHamburgerClick);
