...
type TaxonomyApiRow = { id: string; title: string };
type TaxonomyListResponse = { data?: TaxonomyApiRow[] };
...
export default function AddProductClient({ productId }: AddProductClientProps) {
  ...
  const attributeParents: AttrOption[] = attributeWhitelist
    .filter((x) => x.id)
    .map((x) => ({ id: x.id as string, title: x.value }));
  ...
  async function onChangeAttributeParent(rowId: number, parentAttrId: string) {
    ...
    setAttributeRows((prev) =>
      prev.map((r) =>
        r.id === rowId ? { ...r, values, loadingValues: false } : r,
      ),
    );
  }
  ...
  function onChangeAttributeValue(rowId: number, valueId: string) {
    setAttributeRows((prev) =>
      prev.map((r) => (r.id === rowId ? { ...r, valueId } : r)),
    );
  }
  ...
  function onTaxonomyCreated(t: { id: string; name: string; kind: string }) {
    ...
    if (t.kind === "product_cat") {
      setCategoryWhitelist((p) =>
        p.some((x) => x.id === newItem.id) ? p : [newItem, ...p],
      );
    ...
    if (t.kind === "product_attribute") {
      setAttributeWhitelist((p) =>
        p.some((x) => x.id === newItem.id) ? p : [newItem, ...p],
      );
    }

    if (t.kind === "product_variable") {
      setVariableWhitelist((p) =>
        p.some((x) => x.id === newItem.id) ? p : [newItem, ...p],
      );
    }
  }
  ...
  useEffect(() => {
    ...
    async function loadTaxonomies() {
      try {
        const [catsRes, tagsRes, attrsRes, varsRes] = await Promise.all([
          apiFetch("/api/taxonomy?kind=product_cat", { cache: "no-store" }),
          apiFetch("/api/taxonomy?kind=product_tag", { cache: "no-store" }),
          apiFetch("/api/taxonomy?kind=product_attribute", {
            cache: "no-store",
          }),
          apiFetch("/api/taxonomy?kind=product_variable", {
            cache: "no-store",
          }),
        ]);

        const [catsJson, tagsJson, attrsJson, varsJson] = (await Promise.all([
          catsRes.json(),
          tagsRes.json(),
          attrsRes.json(),
          varsRes.json(),
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setLoadError(errorMessage(e, "خطا در دریافت محصول"));
      } finally {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(errorMessage(e, "خطا در ذخیره"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <>
        ...
        <div className="col-12 col-lg-7">
            ...
            <div className="card-body">
              ...
              <div>
                ...
                <input
                  ...
                  className="form-control"
                  placeholder="نامک خود را وارد کنید"
                />{" "}
              </div>
            </div>
          </div>
          ...
          <div className="card mb-4">
            <div className="nav-align-top">
              <ul
                className="nav nav-pills mb-3 p-4 justify-content-between justify-content-sm-start"
                role="tablist"
              >
                <li>
                  <button
                    type="button"
                    className="nav-link px-2 px-md-4 active"
                    role="tab"
                    data-bs-toggle="tab"
                ...
                <li className="nav-item" role="presentation">
                  <button
                    ...
                    role="tab"
                    data-bs-toggle="tab"
                    data-bs-target="#product-excerpt"
                    aria-controls="product-excerpt"
                    aria-selected="false"
                    tabIndex={-1}
                  >
                    توضیحات کوتاه
              ...
              <div
                className="tab-pane fade px-4 active show"
                id="product-content"
                role="tabpanel"
              >
          ...
          <div className="accordion mb-4" id="collapsibleSection">
            <div className="card">
              ...
              <div className="nav-align-left">
                <ul
                  className="nav nav-tabs tabs-line border-end"
                  role="tablist"
                >
                  <li
                    className={`nav-item ${productType === "variable" ? "d-none" : ""}`}
                    role="presentation"
                  >
                    <button
                      type="button"
                ...
                <div className="tab-content shadow-none p-3">
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder="YYYY/MM/DD تا YYYY/MM/DD"
                      id="flatpickr-range"
                    />
                    <input
                  ...
                  <div className="tab-pane fade" id="tab-stock" role="tabpanel">
                    ...
                    <input
                      ...
                      defaultValue={0}
                      className="form-control mb-3"
                      placeholder="موجودی انبار خود را وارد کنید"
                    />
                    ...
                    <input
                      ...
                      defaultValue={0}
                      className="form-control"
                      placeholder="محدودیت خرید محصول را وارد کنید"
                    />
                  </div>
                    ...
                    <div id="attribute_container">
                      {attributeRows.length === 0 ? (
                        <div className="alert alert-warning mb-4">
                          هنوز ویژگی ای اضافه نشده. روی + بزنید تا اولین ویژگی
                          ساخته شود.
                        </div>
                      ) : null}
                      ...
                      {attributeRows.map((row) => (
                        ...
                        >
                          <div className="col-12 col-md-6">
                            <div className="position-relative">
                              <select
                                id={`mainselect-${row.id}`}
                                className="form-select"
                                value={row.parentAttrId}
                                onChange={(e) =>
                                  void onChangeAttributeParent(
                                    row.id,
                                    e.target.value,
                                  )
                                }
                          ...
                          <div className="col-12 col-md-6">
                            <div className="position-relative">
                              <select
                                ...
                                value={row.valueId}
                                onChange={(e) =>
                                  onChangeAttributeValue(row.id, e.target.value)
                                }
                                disabled={
                                  row.parentAttrId === "delete" ||
                                  row.loadingValues
                                }
                              >
                                <option value="delete">
                  ...
                  >
                    <div id="variables_container" className="list-group mb-3">
                      ...
                      {vars.rows.length === 0 ? (
                        <div className="alert alert-warning mb-4">
                          هنوز متغییری اضافه نشده. روی + بزنید تا اولین متغییر
                          ساخته شود.
                        </div>
                      ) : null}
                      ...
                      {vars.rows.map((row) => (
                        <ul
                          key={row.id}
                          className="list-group-item p-3 mb-0"
                          id={`main-variable${row.id}`}
                        >
                          <button
                            type="button"
                            className="btn-close position-absolute top-0 end-0 m-2"
                            title="حذف متغییر"
                            onClick={() => vars.removeRow(row.id)}
                          />
                          <li className="row variable-attribute-container mt-4">
                            <div className="col-12 col-md-5 mb-3">
                              <div className="position-relative">
                                <select
                                  name={`var[${row.id}][attr-name][]`}
                                  className="form-select"
                                  id={`var${row.id}-name-0`}
                                >
                                  <option value="delete">حذف</option>
                            ...
                            <div className="col-12 col-md-5 mb-3">
                              <div className="position-relative">
                                <select
                                  name={`var[${row.id}][attr-value][]`}
                                  className="form-select"
                                  id={`var${row.id}-value-0`}
                                >
                                  <option value="delete">حذف</option>
                                </select>
                              </div>
                            ...
                            <div className="col-2 mb-3">
                              <button
                                type="button"
                                className="btn btn-primary w-100 text-center px-0 add-var-attr"
                              >
                                <i className="fa fa-plus-square" />
                              </button>
                            </div>
                          ...
                          <li className="row">
                            <div className="col-12 col-md-6">
                              <div className="d-flex flex-wrap align-items-center">
                                <button
                                  type="button"
                                  className="btn-close text-reset ml-2 remove-var-image"
                                  title="حذف تصویر"
                                  onClick={() => vars.clearRowImage(row.id)}
                                />
                                ...
                                <button
                                  type="button"
                                  className="btn btn-sm btn-success mr-auto mb-2 ms-auto"
                                >
                                  انتخاب تصویر
                                </button>
                                ...
                                <div className="w-100">
                                  {row.thumbnail ? (
                                    <img
                                      src={row.thumbnail}
                                      height={150}
                                      className="mt-3 mx-auto d-block mw-100"
                                      alt=""
                                    />
                                  ) : null}
                                </div>
                            ...
                            <div className="col-12 col-md-6">
                              <div className="row">
                                <div className="col-12">
                                  ...
                                  <input
                                    type="number"
                                    name={`var[${row.id}][price]`}
                                    defaultValue=""
                                    className="form-control"
                                    placeholder="قیمت متغییر را وارد کنید"
                                  />
                                </div>
            ...
            <div className="card accordion-item">
              ...
              <div
                id="seo"
                className="accordion-collapse card-body collapse"
              ...
              >
                <div className="mb-3 position-relative">
                  ...
                  <input
                    ...
                    name="seo_title"
                    className="form-control"
                    placeholder="متا تایتل خود را وارد کنید"
                    value={seoTitle.value}
                    onChange={(e) => seoTitle.setValue(e.target.value)}
                  />
                  <div
                    id="seo_title"
                    style={{ position: "absolute", top: 5, left: 10 }}
                  >
                    <span style={{ color: seoTitle.color }}>
                      {seoTitle.len}
                    </span>
                    /<span>65</span>
                  </div>
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    defaultValue=""
                    className="form-control"
                    placeholder="متا کیوردز خود را وارد کنید"
                  />
                </div>
                ...
                <div className="mb-3 position-relative">
                  ...
                  <input
                    ...
                    name="seo_desc"
                    className="form-control"
                    placeholder="متا دیسکریپشن خود را وارد کنید"
                    value={seoDesc.value}
                    onChange={(e) => seoDesc.setValue(e.target.value)}
                  />
                  <div
                    id="seo_desc"
                    style={{ position: "absolute", top: 5, left: 10 }}
                  >
                    <span style={{ color: seoDesc.color }}>{seoDesc.len}</span>/
                    <span>165</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    defaultValue=""
                    className="form-control"
                    placeholder="لینک canonical خود را وارد کنید"
                  />
                </div>
                ...
                <div className="mt-3">
                  ...
                  <input
                    ...
                    defaultValue=""
                    className="form-control"
                    placeholder="لینک schema خود را وارد کنید"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="card accordion-item">
              <h2 className="accordion-header">
                <button
                  ...
                  className="accordion-button collapsed"
                  data-bs-toggle="collapse"
                  data-bs-target="#related_produtcs"
                >
                  محصولات مکمل
                </button>
              </h2>
              ...
              >
                <ProductPickerField
                  controller={complementariesPicker.controller}
                  wrapRef={complementariesPicker.wrapRef}
                  inputName="complementaries"
                />
              </div>
            </div>
            ...
            <div className="card accordion-item">
              <h2 className="accordion-header">
                <button
                  ...
                  className="accordion-button collapsed"
                  data-bs-toggle="collapse"
                  data-bs-target="#replacements_produtcs"
                >
                  محصولات جایگزین
                </button>
              </h2>
              <div
                id="replacements_produtcs"
                className="accordion-collapse card-body collapse"
                data-bs-parent="#collapsibleSection"
              >
                <ProductPickerField
                  controller={replacementsPicker.controller}
                  wrapRef={replacementsPicker.wrapRef}
                  inputName="replacements"
                />
              </div>
            </div>
          </div>
      ...
      <AddTaxonomyOffcanvas
        open={taxonomyModal.open}
        onClose={taxonomyModal.close}
        kind={taxonomyModal.cfg.kind}
        titleLabel={taxonomyModal.cfg.title}
        parentOptions={taxonomyModal.cfg.parentOptions ?? []}
