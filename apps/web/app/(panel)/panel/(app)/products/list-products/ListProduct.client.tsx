...
export default function ListProductClient() {
  ...
  const [dataActive, setDataActive] = useState<GetProductsResponse | null>(
    null,
  );
  ...
  const statusForTab: ProductStatus | undefined =
    activeTab === "published"
      ? "ACTIVE"
      : activeTab === "draft"
        ? "DRAFT"
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const qs = buildQuery({
        ...
        // Add these when the backend supports them:
        // minPrice: minCost,
        // maxPrice: maxCost,
      ...
      const json = (await res.json()) as Partial<GetProductsResponse> & {
        message?: string;
      };
      ...
      setError(errorMessage(e, "خطای نامشخص"));
    } finally {
      setLoading(false);
    }
  }, [activeTab, debouncedSearch, page, selectedCategory, statusForTab]);
    ...
    async function loadTaxonomies() {
      ...
      try {
        const [catRes, brandRes] = await Promise.all([
          apiFetch(`/api/taxonomy?kind=product_cat&limit=200`, {
            cache: "no-store",
          }),
          apiFetch(`/api/taxonomy?kind=product_brand&limit=200`, {
            cache: "no-store",
          }),
        ]);

        const [catJson, brandJson] = await Promise.all([
          catRes.json() as Promise<TaxonomyListResponse>,
          brandRes.json() as Promise<TaxonomyListResponse>,
        ]);

        if (!catRes.ok)
          throw new Error(
            catJson.message || `categories failed (${catRes.status})`,
          );
        if (!brandRes.ok)
          throw new Error(
            brandJson.message || `brands failed (${brandRes.status})`,
          );
        ...
        if (!cancelled) setTaxError(errorMessage(e, "taxonomy load failed"));
      } finally {
            ...
            <div className="card-body border-bottom">
              ...
              <div className="formCost d-flex gap-2 align-items-center mt-3">
                ...
                <input
                  ...
                  value={maxCost}
                  onChange={(e) => {
                    setMaxCost(Number(e.target.value || 0));
                    setPage(1);
                  }}
                />
              </div>
            ...
            <div className="accordion-item">
              <h2 className="accordion-header" id="flush-headingBrands">
                ...
                >
                  ...
                  <span className="badge bg-info rounded-pill align-middle ms-1 filter-badge">
                    ...
                    {[50, 40, 30, 20, 10].map((n) => (
                      <div className="form-check" key={n}>
                        <input
                          ...
                          type="checkbox"
                          id={`discount-${n}`}
                          checked={selectedDiscounts.includes(n)}
                          onChange={() => toggleDiscount(n)}
                        />
                        <label
                          className="form-check-label"
                          htmlFor={`discount-${n}`}
                    ...
                    {[4, 3, 2, 1].map((n) => (
                      <div className="form-check" key={n}>
                        <input
                          ...
                          type="checkbox"
                          id={`rating-${n}`}
                          checked={selectedRatings.includes(n)}
                        ...
                        >
                          <span className="text-muted">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <i
                                key={i}
                                className={`mdi mdi-star ${i < n ? "text-warning" : ""}`}
                              />
                            ))}
                          </span>
                          {n} و بالاتر
          ...
          <div className="card">
            <div className="card-header border-0">
              <div className="row g-4">
                <div className="col-sm-auto">
                  <div>
                    <Link
                      href="/panel/products/add-product"
              ...
              <div className="row align-items-center">
                <div className="col">
                  ...
                  >
                    <li className="nav-item">
                      <button
                        type="button"
                        className={`nav-link text-body fw-semibold ${activeTab === "all" ? "active" : ""}`}
                        onClick={() => {
                          setActiveTab("all");
                          setPage(1);
                        }}
                    ...
                    <li className="nav-item">
                      <button
                        type="button"
                        className={`nav-link text-body fw-semibold ${activeTab === "published" ? "active" : ""}`}
                        onClick={() => {
                    ...
                    <li className="nav-item">
                      <button
                        type="button"
                        className={`nav-link text-body fw-semibold ${activeTab === "draft" ? "active" : ""}`}
                        onClick={() => {
                          setActiveTab("draft");
                          setPage(1);
                        }}
                ...
                <div className="col-auto">
                  {loading ? (
                    ...
                    <span className="text-muted">
                      صفحه {currentData.page} از {currentData.totalPages} —{" "}
                      {currentData.total} نتیجه
                    </span>
                  ) : null}
                </div>
              </div>
            ...
            <div className="card-body">
              ...
              <div className="tab-content text-muted">
                ...
                <div
                  className={`tab-pane ${activeTab === "published" ? "active" : ""}`}
                ...
                <div
                  className={`tab-pane ${activeTab === "draft" ? "active" : ""}`}
                  id="productnav-draft"
                  role="tabpanel"
                >
                  <ProductsTable
                    items={items}
                    loading={loading}
                    emptyText="متاسفم! هیچ نتیجه ای یافت نشد"
                  />
                  <Pager
                    page={currentData?.page ?? page}
                    totalPages={currentData?.totalPages ?? 1}
            ...
            <tr key={p.id}>
              ...
              <td>
                <div className="fw-semibold text-body">
                  <Link
                    href={`/panel/products/add-product/${p.id}/edit`}
              ...
              <td>
                <span
                  className={`badge ${p.status === "ACTIVE" ? "bg-success-subtle text-success" : p.status === "DRAFT" ? "bg-warning-subtle text-warning" : "bg-secondary-subtle text-secondary"}`}
                >
                  {p.status === "ACTIVE"
                    ? "فعال"
                    : p.status === "DRAFT"
              ...
              <td>
                {p.isFeatured ? (
                  <span className="badge bg-info-subtle text-info">ویژه</span>
                ) : (
                  "-"
                )}
              </td>
