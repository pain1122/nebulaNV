...
export const GATEWAY_DEFAULT_IDEMPOTENCY_TTL_SECONDS = 24 * 60 * 60;
...
export const envSchema = Joi.object({
  ...
  GATEWAY_IDEMPOTENCY_TTL_SECONDS: Joi.number()
    .integer()
    .min(60)
    .max(7 * 24 * 60 * 60)
    .default(GATEWAY_DEFAULT_IDEMPOTENCY_TTL_SECONDS),
  GATEWAY_IDEMPOTENCY_IN_FLIGHT_TTL_SECONDS: Joi.number()
    .integer()
    .min(5)
    .max(10 * 60)
    .default(GATEWAY_DEFAULT_IDEMPOTENCY_IN_FLIGHT_TTL_SECONDS),
  GATEWAY_IDEMPOTENCY_MAX_RESPONSE_BYTES: Joi.number()
    .integer()
    .min(1024)
    .max(2 * 1024 * 1024)
    .default(GATEWAY_DEFAULT_IDEMPOTENCY_MAX_RESPONSE_BYTES),
});
