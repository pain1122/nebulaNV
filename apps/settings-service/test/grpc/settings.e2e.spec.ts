...
describe('SettingsService gRPC (gateway-only S2S, svc:bucket)', () => {
  ...
  it('SetString then GetString (hit) → found=true, value matches', async () => {
    const set = await call<any>(
      client,
      'SetString',
      { namespace: ns, environment: env, key, value: 'Nebula E2E' },
      mdS2S()
    );
  ...
  it('Missing signature → request fails', async () => {
    await expect(
      call<any>(client, 'SetString', { namespace: ns, environment: env, key, value: 'x' })
    ).rejects.toBeTruthy();
  });
});
