...
import {firstHeaderValue, firstMetadataValue, getContextService, getContextUser, type ContextCarrier, type ContextUser, type HttpRequestWithContext, type MetadataWithContext, type RpcContextWithContext} from "./context"
...
export class GrpcTokenAuthGuard implements CanActivate, OnModuleInit {
  ...
  private getSvc(ctx: ExecutionContext): string | null {
    const req = ctx.switchToHttp().getRequest<HttpRequestWithContext | undefined>()
    const reqSvc = getContextService(req)
    if (reqSvc) return reqSvc

    const call = ctx.switchToRpc().getContext<RpcContextWithContext | undefined>()
  ...
  private extractToken(ctx: ExecutionContext): string | null {
    const httpReq = ctx.switchToHttp().getRequest<HttpRequestWithContext | undefined>()
    let token = this.parseBearer(firstHeaderValue(httpReq?.headers?.[AUTHORIZATION_HEADER]))
    if (!token) {
  ...
  private getCtxUser(ctx: ExecutionContext): ContextUser | null {
    ...
    return getContextUser(req) ?? getContextUser(call) ?? getContextUser(this.extractMeta(ctx)) ?? getContextUser(contextCarrier) ?? null
  }
  ...
  private async attachUserFromToken(ctx: ExecutionContext, token: string): Promise<void> {
