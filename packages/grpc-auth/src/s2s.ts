 ...
 */
export function buildS2SMetadata(opts?: {serviceName?: string; secret?: string; headerName?: string; upstreamSvc?: string; method?: string; path?: string}): Metadata {
  ...
  if (secret) {
    const sig = signS2S(derived, svc, S2S_METHOD_CANONICAL, S2S_PATH_CANONICAL, minuteBucket())
    md.set(header, sig)
  }
