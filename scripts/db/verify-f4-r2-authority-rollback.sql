-- A fresh psql connection must see only the exact pre-R2 schema and rows.
DO $$
BEGIN
  IF EXISTS (
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM "IdentityRealm") OR (SELECT count(*) FROM "Membership") <> 1 THEN
    RAISE EXCEPTION 'r2_authority_rollback_fixture_changed';
  END IF;
END $$;
