-- R2 order: exact pairs, zero-row rerun, all legacy/business columns preserved.
BEGIN;
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['Order', 'Cart'] LOOP
    EXECUTE format(
    ...
    IF bad THEN RAISE EXCEPTION 'r2_order_pair_mismatch: %', table_name; END IF;
    EXECUTE format(
    ...
    IF affected <> 0 THEN RAISE EXCEPTION 'r2_order_rerun_wrote_rows'; END IF;

    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "identityRealmId" = NULL WHERE %I IS NOT NULL', table_name, 'userId'), 'order_default_actor_pair_partial');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "subjectId" = NULL WHERE %I IS NOT NULL', table_name, 'userId'), 'order_default_actor_pair_partial');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "identityRealmId" = %L WHERE %I IS NOT NULL', table_name, 'b1000000-0000-4000-8000-000000000002', 'userId'), 'order_default_actor_pair_mismatch');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "subjectId" = %L WHERE %I IS NOT NULL', table_name, 'd1000000-0000-4000-8000-000000000099', 'userId'), 'order_default_actor_pair_mismatch');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET %I = NULL WHERE %I IS NOT NULL', table_name, 'userId', 'userId'), '23502');
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['Order', 'Cart'] LOOP
    ...
    THEN RAISE EXCEPTION 'r2_order_legacy_writer_pair_missing'; END IF;
  END LOOP;
END $$;
ROLLBACK;
