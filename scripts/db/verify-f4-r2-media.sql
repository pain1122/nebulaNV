-- R2 media: exact pairs, zero-row rerun, all legacy/business columns preserved.
BEGIN;
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['Media'] LOOP
    EXECUTE format(
      ...
      'ownerId', 'b1000000-0000-4000-8000-000000000001', 'ownerId', table_name
    ) INTO bad;
    IF bad THEN RAISE EXCEPTION 'r2_media_pair_mismatch: %', table_name; END IF;
    EXECUTE format(
      'UPDATE %I SET %I = %I WHERE %I IS NOT NULL AND "identityRealmId" IS NULL AND "subjectId" IS NULL',
      table_name, 'ownerId', 'ownerId', 'ownerId'
    );
    GET DIAGNOSTICS affected = ROW_COUNT;
    IF affected <> 0 THEN RAISE EXCEPTION 'r2_media_rerun_wrote_rows'; END IF;

    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "identityRealmId" = NULL WHERE %I IS NOT NULL', table_name, 'ownerId'), 'media_default_actor_pair_partial');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "subjectId" = NULL WHERE %I IS NOT NULL', table_name, 'ownerId'), 'media_default_actor_pair_partial');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "identityRealmId" = %L WHERE %I IS NOT NULL', table_name, 'b1000000-0000-4000-8000-000000000002', 'ownerId'), 'media_default_actor_pair_mismatch');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "subjectId" = %L WHERE %I IS NOT NULL', table_name, 'd1000000-0000-4000-8000-000000000099', 'ownerId'), 'media_default_actor_pair_mismatch');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET %I = NULL WHERE %I IS NOT NULL', table_name, 'ownerId', 'ownerId'), 'media_default_actor_pair_without_owner');
    PERFORM pg_temp.r2_reject(format('UPDATE %I SET "identityRealmId" = %L, "subjectId" = %L WHERE %I IS NULL', table_name, 'b1000000-0000-4000-8000-000000000001', 'd1000000-0000-4000-8000-000000000099', 'ownerId'), 'media_default_actor_pair_without_owner');
  END LOOP;
END $$;
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['Media'] LOOP
    EXECUTE format('SELECT to_jsonb(t) FROM %I t WHERE %I IS NOT NULL LIMIT 1', table_name, 'ownerId') INTO probe;
    probe := probe || jsonb_build_object(
      'id', 'f1000000-0000-4000-8000-000000000001',
      'ownerId', 'd1000000-0000-4000-8000-000000000099',
    ...
    IF actual->>'identityRealmId' IS DISTINCT FROM 'b1000000-0000-4000-8000-000000000001'
      OR actual->>'subjectId' IS DISTINCT FROM actual->>'ownerId'
    THEN RAISE EXCEPTION 'r2_media_legacy_writer_pair_missing'; END IF;
  END LOOP;
END $$;
ROLLBACK;
